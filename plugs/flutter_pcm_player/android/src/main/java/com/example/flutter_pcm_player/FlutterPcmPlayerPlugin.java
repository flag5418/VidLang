...
public class FlutterPcmPlayerPlugin implements FlutterPlugin, MethodCallHandler {
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
    if (call.method.equals("initialize")) {
      ...
      result.success(player.getPlayState());
    } else if (call.method.equals("setGlobalAudioSession")) {
      // Android平台不需要全局音频会话配置，直接返回成功
      // Android的音频管理机制与iOS不同，无需强制采样率
      Log.d("PcmPlayer", "setGlobalAudioSession() called (Android - no action needed)");
      result.success(0);
    } else {
