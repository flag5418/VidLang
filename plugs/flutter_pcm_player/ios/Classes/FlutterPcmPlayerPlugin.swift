...
public class FlutterPcmPlayerPlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
   var args = [String: AnyObject]()
       ...
       case "setGlobalAudioSession":
         setGlobalAudioSession(result: result)
       default:
   ...
   private func sendResultError(
    ...
    private func setGlobalAudioSession(result: @escaping FlutterResult) {
      do {
        let session = AVAudioSession.sharedInstance()
        // 使用 Playback 类别（不录音，只播放），确保音量正常
        try session.setCategory(.playback, mode: .default, options: [.allowBluetooth, .allowAirPlay])
        try session.setActive(true)
        // 设置系统音量最大（用户仍可通过物理按键调节）
        // 注意：MPVolumeView 可以设置音量，但这里我们只确保音频会话正确
        sendResultInt(0, result: result)
      } catch {
        sendResultError(
          "Error", message: "Failed to set global audio session: \(error.localizedDescription)", details: nil, result: result)
      }
    }
}
