...
class FlutterPcmPlayer {
  ...
  Future<void> play() async {
    ...
    var state = await _channel.invokeMethod("play");
  ...
  Future<void> stop() async {
    ...
    var state = await _channel.invokeMethod("stop");
    _playState = PlayState.values[state];
    if (_playState == PlayState.playing) {
  ...
  Future<void> pause() async {
    _ensureInited();
    var state = await _channel.invokeMethod("pause");
    _playState = PlayState.values[state];
  ...
  Future<void> resume() async {
    _ensureInited();
    var state = await _channel.invokeMethod("resume");
    _playState = PlayState.values[state];
    if (_playState != PlayState.playing) {
  ...
  Future<void> feed(Uint8List data) async {
    ...
    var state = await _channel.invokeMethod("feed", {
      'data': data,
    });
    _playState = PlayState.values[state];
  }
  ...
  Future<void> setVolume(double volume) async {
    _ensureInited();
    var state = await _channel.invokeMethod("setVolume", {
      'volume': volume,
    });
    _playState = PlayState.values[state];
  }

  /// Sets the global audio session configuration (iOS only).
  /// Enforces PlayAndRecord category, Default mode, and 48000Hz sample rate.
  /// This helps prevent crashes when switching between recording and playback.
  /// Note: This is only applicable to iOS. Android handles audio sessions differently.
  static Future<void> setGlobalAudioSession() async {
    try {
      await _channel.invokeMethod("setGlobalAudioSession");
    } catch (e) {
      print("FlutterPcmPlayer: Failed to set global audio session: $e");
    }
  }
