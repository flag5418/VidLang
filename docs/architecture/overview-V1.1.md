...
> **状态**: 当前有效 | **变更**: 移除本地模型引用、同步订阅模式严格分流、Android 强制 premium
...
| iOS 原生能力 | Apple MLTranslation / AVSpeech | 免费模式翻译/TTS（仅 iOS） |
...
| 场景 | 通道 | 说明 |
|------|------|------|
| 查词释义（ai_definition） | **Edge Function** (`ai-proxy`) / iOS MLTranslation | Premium 走云端（计费+缓存），Free(iOS) 走系统原生 |
| AI 翻译（ai_translate） | **Edge Function** (`ai-proxy`) / iOS MLTranslation | 同上，严格按模式分流 |
| TTS 语音合成 | **DashScope HTTP SSE 直连** / **AVSpeechSynthesizer** | Premium: DashScope（本地缓存）；Free(iOS): 系统原生 |
| STT 发音评分 | **声通 WebSocket 直连** (`ShengtongEvaluator`) | 仅 Premium 可用 |
| AI 对话（conversation） | **Qwen Realtime WebSocket** + Edge Function 创建会话 | 仅 Premium 可用 |
...
2. **Offline First** — 本地 SQLite 为核心存储，必要数据按需上传云端
3. **Open Content** — 不依赖平台内容库，用户自由导入
4. **Freemium** — 免费(iOS 原生能力)获客，付费层(AI + 评测)变现；Android 强制付费
