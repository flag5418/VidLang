...
> 充值采用**赠送模式**（非折扣模式），由 `topup_config` 表驱动，后端可随时调整优惠策略。
...
> 代码兜底数据（`TopupService._fallbackConfigs()`）与上表一致，当 `topup-config` Edge Function 不可用时自动降级。
...
| 路径 | 入口 | 协议 | 使用状态 |
|------|------|------|---------|
| **主力路径** | `TtsService` → `UnifiedTtsService` → `DashScopeTtsService` | DashScope HTTP SSE 直连（`qwen3-tts-flash`） | ✅ **活跃使用** |
| **备用路径** | `AiService.getTtsAudio()` → `ai-proxy` (`ai_tts`) | Edge Function 内路由到 qwenTts | ⚠️ **已无调用者（死代码）** |

### 为什么保留 `ai_tts` 路由

- `ai-proxy` Edge Function 中仍保留 `ai_tts` 路由和 `qwenTts`/`qwenTtsStreaming` 实现
- `AiService.getTtsAudio()` 方法仍存在于代码中
- 原因：作为降级方案保留，未来如果需要通过 Edge Function 统一计费 TTS，可快速激活
- **注意**: 当前所有 TTS 调用（播放器朗读、清晰朗读、单词发音）均走 DashScope 直连路径，不经过 Edge Function
...
| 5 | **新增** `billing-center/index.ts` | 聚合查询接口（8 种 op 模式：overview/by_category/category_detail/by_source/source_detail/action_details/resource_type_details/resource_details） | ✅ **已完成** | 中 |
...
| # | 内容 | 优先级 | 说明 |
|---|------|------|------|
| 5 | `billing-center` 替代原设计的 `usage-stats` | 已实现 8 种聚合查询 op（超原设计 5 种） | ✅ **已完成** |
...
P1（近期 - 账单展示）：
├── #5    `billing-center` Edge Function（已完成 8 种 op）
...
- `billing-center` 为已实现的聚合查询接口，不影响现有功能
...
- [数据库设计](../architecture/database-schema-V2.0.md) - 云端表结构（pricing_rule / billing_records / topup_config / wallet_ledger / user_wallet）
- [Supabase 集成](../reference/supabase-integration.md) - Edge Functions 清单与多通道 AI 架构
- [服务层架构](../reference/services-architecture.md) - BillingService / TopupService 服务详解
