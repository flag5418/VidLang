...
void main() {
  ...
  final Map<String, WordRelation> wordRelations = {
    ...
    'beautiful': WordRelation(
      synonyms: ['pretty', 'lovely', 'attractive'],
      antonyms: ['ugly'],
      category: 'appearance',
    ),
    'new': WordRelation(
      synonyms: ['fresh', 'modern'],
  ...
  for (var i = 0; i < 3; i++) {
    final wordPool = extractWordPool(
      testSentences,
      difficultyParamsMap['intermediate']!,
    );
    final items = pickDefinitionChoiceItemsOptimized(
      testSentences,
      wordPool,
      2,
      wordMeaningMap,
    ...
    for (final item in items) {
      if (item['sub_type'] == 'cn_to_en') {
        ...
        // 验证提示词中不包含英文答案
        final noAnswerInPrompt = !promptCn.toLowerCase().contains(
          answer.toLowerCase(),
        );
        // 验证显示文本是中文释义而非英文单词
  ...
  for (var i = 0; i < 3; i++) {
    ...
    for (final item in items) {
      ...
      final chineseOptionCount = options
          .where((opt) => RegExp(r'[\u4e00-\u9fa5「」；、]').hasMatch(opt))
          .length;
  ...
  for (var i = 0; i < 2; i++) {
    ...
    final items = pickTranslateMeaningItemsOptimized(
      testSentences,
      wordPool,
      1,
      difficultyParamsMap['intermediate']!,
    ...
    for (final item in items) {
      ...
      // 验证选项不全是英文句子
      final notAllEnglish = !options.every(
        (opt) => RegExp(r"^[A-Za-z\s.,!?'-]+$").hasMatch(opt),
      );
  ...
  for (var i = 0; i < 3; i++) {
    ...
    for (final item in items) {
      ...
      final answers =
          (item['answers'] as List?)?.map((a) => a.toString()).toList() ?? [];
      final displayText = item['display_text']?.toString() ?? '';
  ...
  for (var i = 0; i < 3; i++) {
    ...
    final items = pickListenReplyItemsOptimized(
      testSentences,
      wordPool,
      1,
      difficultyParamsMap['intermediate']!,
  ...
  for (final diff in difficulties) {
    ...
    print(
      '--- 难度: $diff (单词长度 ${params.minWordLen}-${params.maxWordLen}, 句子长度 ≤${params.maxSentenceLen}) ---',
    );
    ...
    if (reorderItems.isNotEmpty) {
      ...
      final validLength =
          wordCount >= params.minWords && wordCount <= params.maxWords;
      runTest(
        '重组句题单词数: $wordCount (范围 ${params.minWords}-${params.maxWords})',
        validLength,
      );
    }
    ...
    if (spellingItems.isNotEmpty) {
      ...
      final validLen =
          answer.length >= params.minWordLen &&
          answer.length <= params.maxWordLen;
      runTest(
        '拼写题单词长度: "$answer" (${answer.length}, 范围 ${params.minWordLen}-${params.maxWordLen})',
        validLen,
      );
    }
  }
  ...
  allItems.addAll(
    pickDefinitionChoiceItemsOptimized(
      testSentences,
      wordPool,
      2,
      wordMeaningMap,
  ...
  allItems.addAll(
    pickTranslateMeaningItemsOptimized(
      testSentences,
      wordPool,
      1,
      params,
  ...
  print(
    '通过: ✅ $passedTests (${(passedTests / totalTests * 100).toStringAsFixed(1)}%)',
  );
  print(
    '失败: ❌ $failedTests (${(failedTests / totalTests * 100).toStringAsFixed(1)}%)',
  );
...
List<String> getDistractorMeanings(
  String targetWord,
  List<String> wordPool,
  int count,
  Map<String, String> meaningMap,
) {
  final others = shuffle(
    wordPool.where((w) => w.toLowerCase() != targetWord.toLowerCase()).toList(),
  );
...
/// 优化版：听音选词
List<Map<String, dynamic>> pickListenChooseItemsOptimized(
  List<String> wordPool,
  int count,
) {
  final items = <Map<String, dynamic>>[];
...
/// 优化版：听音辩义（使用中文释义）
List<Map<String, dynamic>> pickListenMeaningItemsOptimized(
  List<String> wordPool,
  int count,
  Map<String, String> meaningMap,
) {
  ...
  for (final w in shuffle(wordPool)) {
    ...
    final distractorMeanings = getDistractorMeanings(
      w,
      wordPool,
      3,
      meaningMap,
...
/// 优化版：听音回复
List<Map<String, dynamic>> pickListenReplyItemsOptimized(
  List<String> sentences,
  List<String> wordPool,
  int count,
  DifficultyParams params,
  ...
  for (final s in shuffle(candidates)) {
    ...
    final isQuestion = RegExp(
      r'^(what|where|when|why|how|who|which|can|could|would|do|does|did|is|are|am)',
      caseSensitive: false,
    ).hasMatch(s);
      ...
      final nouns = words
          .where(
            (w) =>
                ...
                nounKeywords.contains(w.toLowerCase()),
          )
          .toList();
      keyWord = nouns.isNotEmpty
          ? nouns.first
          : words[min(2, words.length - 1)];
    }

    final keyWordLower = keyWord.toLowerCase();
    final distractors = shuffle(
      wordPool.where((x) => x != keyWordLower).toList(),
    ).take(3).toList();
    if (distractors.length < 3) continue;

    final correctAnswer = getWordMeaning(keyWordLower, meaningMap);
    final distractorAnswers = distractors
        .map((d) => getWordMeaning(d, meaningMap))
        .toList();
...
/// 优化版：释义选择（修复答案泄露）
List<Map<String, dynamic>> pickDefinitionChoiceItemsOptimized(
  List<String> sentences,
  List<String> wordPool,
  int count,
  Map<String, String> meaningMap,
  ...
  for (final w in shuffle(wordPool)) {
    ...
    if (isEnglishToChinese) {
      ...
      final englishOptions = shuffle(
        wordPool.where((x) => x != w).toList().take(3).toList(),
      );
...
/// 优化版：英义互译（使用模拟翻译）
List<Map<String, dynamic>> pickTranslateMeaningItemsOptimized(
  List<String> sentences,
  List<String> wordPool,
  int count,
  DifficultyParams params,
  ...
  for (final s in shuffle(candidates)) {
    ...
    final distractorTranslations = otherSentences
        .map((sent) => mockTranslate(sent))
        .toList();
...
/// 优化版：词性测试（使用预定义语义关系）
List<Map<String, dynamic>> pickWordRelationItemsOptimized(
  List<String> wordPool,
  int count,
  Map<String, WordRelation> relations,
) {
  final items = <Map<String, dynamic>>[];
  final relationTypes = [
    {
  ...
  for (final w in shuffle(wordPool)) {
        ...
        case 'same_category':
          correctAnswers = wordPool
              .where((x) {
                ...
                return xr?.category == wordRelation.category &&
                    x.toLowerCase() != wLower;
              })
              .take(2)
              .toList();
          break;
    ...
    final validCorrectAnswers = correctAnswers
        .where(
          (a) =>
              wordPool.contains(a) ||
              wordPool.any((x) => x.toLowerCase() == a.toLowerCase()),
        )
        .toList();
    ...
    final distractors = shuffle(
      wordPool
          .where(
            (x) =>
                x != w &&
                !validCorrectAnswers.any(
                  (a) => a.toLowerCase() == x.toLowerCase(),
                ),
          )
          .toList(),
    ).take(4).toList();
    ...
    final answerIndices = validCorrectAnswers
...
void _printItemDetails(Map<String, dynamic> item) {
  ...
  switch (type) {
    ...
    case 'listen_reply':
      ...
      if (item.containsKey('answer_word')) {
        print('  关键词: ${item['answer_word']}');
      }
      print('  选项: ${item['options']}');
    ...
    case 'definition_choice':
      ...
      print('  答案: ${item['answer']} (索引: ${item['answer_index']})');
      if (item.containsKey('answer_word')) {
        print('  原始单词: ${item['answer_word']}');
      }
      if (item.containsKey('answer_meaning')) {
        print('  对应释义: ${item['answer_meaning']}');
      }
      break;
