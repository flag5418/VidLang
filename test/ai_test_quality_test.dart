...
const Map<String, DifficultyParams> DIFFICULTY_PARAMS = {
  ...
  'elementary': DifficultyParams(
    minWords: 3, maxWords: 10, minWordLen: 3, maxWordLen: 7, maxSentenceLen: 60,
  ),
  'intermediate': DifficultyParams(
    minWords: 3, maxWords: 14, minWordLen: 3, maxWordLen: 14, maxSentenceLen: 100,
  ),
  'advanced': DifficultyParams(
    minWords: 5, maxWords: 18, minWordLen: 4, maxWordLen: 14, maxSentenceLen: 150,
  ),
  'professional': DifficultyParams(
    minWords: 6, maxWords: 22, minWordLen: 5, maxWordLen: 16, maxSentenceLen: 200,
  ),
};
...
List<Map<String, dynamic>> pickReorderItems(
  ...
  final picked = shuffleList(candidates.map((x) => x['s'] as String).toList()).take(count);
...
List<Map<String, dynamic>> pickMcqItems(
  ...
  for (final w in pool) {
    ...
    final distractors = shuffleList(wordPool.where((x) => x != w).toList()).take(3).toList();
...
List<Map<String, dynamic>> pickListenReplyItems(
  ...
  for (final s in shuffleList(candidates)) {
    ...
    final distractors = shuffleList(wordPool.where((x) => x != keyWord).toList()).take(3).toList();
    if (distractors.length < 3) continue;
...
List<Map<String, dynamic>> pickTranslateMeaningItems(
  ...
  for (final s in shuffleList(candidates)) {
    ...
    final otherSentences = shuffleList(
      sentences.where((x) => x != s && x.length <= params.maxSentenceLen).toList(),
    ).take(3).toList();
...
List<Map<String, dynamic>> pickWordRelationItems(
  ...
  for (final w in shuffleList(wordPool)) {
    ...
    final answerIndices = correctAnswers.map((a) => options.indexOf(a)).toList();
...
/// 跟读单词
List<Map<String, dynamic>> pickWordPronItems(
  List<String> wordPool,
  int count,
) {
  final items = <Map<String, dynamic>>[];
...
List<Map<String, dynamic>> pickSentencePronItems(
  ...
  final items = <Map<String, dynamic>>[];
  final candidates = sentences
      .where((s) => s.length <= params.maxSentenceLen && tokenizeWords(s).length >= params.minWords)
      .map((s) => s.trim())
      .toList();
...
bool checkPromptNoAnswerLeak(Map<String, dynamic> item) {
  ...
  final answer = item['answer']?.toString() ?? item['correct_answer']?.toString() ?? '';
...
void _testAnswerQuality(QualityReport report) {
  ...
  report.add(TestResult(
    ...
    message: promptLeakCount == 0 ? '所有 Prompt 均无答案泄露' : '$promptLeakCount 道 Prompt 存在答案泄露',
  ));
  ...
  report.add(TestResult(
    ...
    message: poorDistractorCount == 0 ? '所有干扰项质量良好' : '$poorDistractorCount 道题干扰项过于相似',
  ));
}
