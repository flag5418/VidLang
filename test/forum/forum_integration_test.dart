...
void main() {
  ...
  group('Forum UI Component Tests', () {
    testWidgets('Forum Home Page Renders Correctly', (
      WidgetTester tester,
    ) async {
      // 这个测试需要在实际应用中运行
      // 现在我们验证组件可以正常构建
...
void testForumPostModel() {
  final postJson = {
    ...
    'category': {'name': 'Discussion', 'slug': 'discussion'},
  ...
  // 测试copyWith方法
  final updatedPost = post.copyWith(title: 'Updated Title', likeCount: 20);
...
void testDatabaseSchema() {
  ...
  for (final field in requiredPostFields) {
    expect(
      samplePostData.containsKey(field),
      isTrue,
